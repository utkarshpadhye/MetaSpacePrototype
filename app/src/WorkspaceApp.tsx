...
import { sections, type WorldObject } from './canvas/world'
...
import { CrmWorkspaceOverlay } from './components/CrmWorkspaceOverlay'
import { BannerNotice } from './components/BannerNotice'
import { useAnaAgent } from './hooks/useAnaAgent'
import { useConferenceWebRTC } from './hooks/useConferenceWebRTC'
import { useConferenceTranscription } from './hooks/useConferenceTranscription'
...
function WorkspaceApp({ workspaceId, userId, userName, permissions, bannerMessage }: Props) {
  ...
  const session = useSessionPresence(displayName)
  ...
  const [roomTransition, setRoomTransition] = useState<'idle' | 'in' | 'out'>('idle')
  ...
  const visibleParticipants = participants.filter((participant) => participant.inSameRoom)
  ...
  const localSectionName = (() => {
    const match = /^Voice:\s*(.*?)\s*\(/.exec(voiceLabel)
    return match?.[1]?.trim() || 'Main Room'
  })()
  const localSectionId = sections.find((section) => section.name === localSectionName)?.id ?? 'main'
  const peoplePanelParticipants = [
    {
      id: session.localPeerId,
      name: `${session.localName} (You)`,
      muted: isMuted,
      cameraOff: false,
      nearby: true,
      distance: 0,
  ...
  const conferenceTranscription = useConferenceTranscription({
    active: isConferenceRoom,
    sessionId: session.sessionId,
    localPeerId: session.localPeerId,
    localName: session.localName,
    sourceStream: conferenceCall.localStream,
  })
  ...
  useEffect(() => {
    if (!isConferenceRoom && screenShare.isSharing) {
      screenShare.stopSharing()
    }
  }, [isConferenceRoom, screenShare.isSharing, screenShare.stopSharing])

  useEffect(() => {
    if (!isConferenceRoom) {
      conferenceCall.setSharedVideoTrack(null)
      return
    }
    const sharedVideoTrack = screenShare.stream?.getVideoTracks()[0] ?? null
    conferenceCall.setSharedVideoTrack(sharedVideoTrack)
  }, [conferenceCall.setSharedVideoTrack, isConferenceRoom, screenShare.stream])
  ...
  useEffect(() => {
    setRoomName(localSectionName)
  }, [localSectionName])

  useEffect(() => {
    const channel = new BroadcastChannel(`metaspace-chat-${session.sessionId}`)
    chatChannelRef.current = channel

    channel.onmessage = (event: MessageEvent<SharedChatMessagePayload>) => {
      const payload = event.data
      if (!payload || payload.type !== 'chat-message') {
        return
      }
      if (payload.sessionId !== session.sessionId || payload.senderId === session.localPeerId) {
        return
      }
  ...
  }, [activePanel, session.localPeerId, session.sessionId])
  ...
  const handleEmojiReaction = useCallback((emoji: string) => {
    const now = Date.now()
    setEmojiReactions((prev) => [...prev, { id: `e-${now}`, emoji, createdAt: now }])
  }, [])
  ...
  const handleToggleParticipants = useCallback(() => {
    ...
    setIsDocsOpen(false)
    setIsCrmOpen(false)
    setActivePanel((current) => (current === 'participants' ? null : 'participants'))
  }, [])
  ...
  const handleSendMessage = useCallback(
    (text: string) => {
      ...
      const message: ChatMessage = {
        id: `m-${session.localPeerId}-${now}-${Math.random().toString(36).slice(2, 6)}`,
      ...
      chatChannelRef.current?.postMessage({
        ...
        senderId: session.localPeerId,
        message,
      } satisfies SharedChatMessagePayload)
    ...
    [session.localName, session.localPeerId, session.sessionId],
  )
  ...
  useEffect(() => {
    const interval = window.setInterval(() => {
      const nearby = participants.filter((participant) => participant.nearby && participant.inSameRoom)
      if (nearby.length === 0) {
  ...
  useEffect(() => {
    const latestHigh = notifications.items.find((item) => item.priority === 'high' && !item.is_read)
    if (!latestHigh) {
      return
    }
    pushToast(`HIGH: ${latestHigh.title}`)
  }, [notifications.items, pushToast])
  ...
  return (
    <div className="min-h-screen bg-[var(--pixel-bg)] text-[var(--pixel-ink)]">
      ...
      <TopBar
        ...
        chatBadgePulse={notifications.highPriorityPulse}
      ...
      {isConferenceRoom ? (
        <ConferenceCallOverlay
          ...
          onToggleCaptions={() => conferenceTranscription.setCaptionsEnabled(!conferenceTranscription.captionsEnabled)}
          onClearTranscript={conferenceTranscription.clearTranscript}
        />
      ) : (
      ...
      <ProjectManagementOverlay open={isProjectsOpen} onClose={() => setIsProjectsOpen(false)} canAccessPm={currentUserRole.permissions.has('room.pm_access')} />
      <DocsWorkspaceOverlay
        open={isDocsOpen}
        onClose={() => setIsDocsOpen(false)}
        canAccessDocs={currentUserRole.permissions.has('doc.view')}
        currentUser={{ name: session.localName, color: '#f97316' }}
      />
      <CrmWorkspaceOverlay
        open={isCrmOpen}
        onClose={() => setIsCrmOpen(false)}
        canAccessCrm={currentUserRole.permissions.has('room.crm_access')}
      />
      {roomTransition !== 'idle' ? (
        <div
